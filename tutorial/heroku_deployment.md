...
- [Firebase](https://firebase.google.com/) - Платформа, которая даёт свой фремворк для взаимодействия. Вы просто пишите **код для этой платформы** и он вызывается платформой тогда, когда это надо, без лишних прогреваний или запуска виртуального сервера. _Оплата по количеству запросов_.
- [Amazon Web Services ECS](https://aws.amazon.com/ru/ecs/) (AWS) - Вы создаёте своё приложение каким хотите и **упаковываете его вместе с настройками** и отдаёте это всё Амазону. _Оплата по времени работы и объёму*время хранения данных в хранилищах_.
- [Heroku](https://heroku.com) - Вы создаёте приложение на [любимом языке](https://devcenter.heroku.com/) и **рассказываете в небольшом файле `Procfile`** как именно ваше приложение можно запустить. _Оплата по времени работы_. (Сам Heroku использует для своих нужд AWS)

Теперь мы понимаем в чём отличия: кто-то просит писать определённый код под себя, кто-то просит делать специальные архивы со специальными настройками, а кто-то просит только 1 файл с инструкциями по запуску.
