...
Для него неоч:
- Бекэнд для мобильных приложений - тут лучше посмотреть в сторону Firebase, там для мобильных приложений очень много интересных плюшек предусмотрено.
...
Давайте сделаем файл-индикатор. В нашем случае (Python) файлом-индикатором является `requirements.txt`, в котором должны храниться названия и версии всех пакетов, которые необходимы для работы нашего приложения. Созать этот файл очень просто:
```bash
