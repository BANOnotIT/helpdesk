...
VDS/VPS - выделенный виртуальный сервер. На каком-то железном сервере поднимают виртуальную операционную систему и дают вам доступ к этой самой виртуальной системой. Внутри можно делать всё что угодно - на железный сервер никак это не повлияет.

Отличие состоит в том, что все облачные сервисы не дают доступ к серверу. Они принимают только контейнеры - специальные файлы с исходниками и настройками для приложения и запускают их в автоматическом режиме без участия человека. Это нужно как раз для того, чтобы хостер мог без особых трудностей развернуть для вас 300 серверов за 10 секунд и точно так же за 2 секунды без лишних проблем отдать эти сервера другому человеку, если они ему будут нужнее.

Каждый сервис предлагает свои способы организации контейнеризации:
...
Buildpack - полуфабрикат для какого-то контейнера. В хероку в этих buildpack'ах спрятана разная магия по установке интерпретаторов языков, установке зависимостей и разного такого интересного.
...
Как я уже говорил, у Heroku есть разные buildpack'и для разных языков и платформ. Чтобы узнать, какой из этих "пресетов" использовать, Heroku полагается на файлы-идентификаторы.
